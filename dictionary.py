...
class Dictionary(SilpaModule):
    ...
    def get_response(self):
        if self.imageyn != None:
            if self.imageyn.lower() == "y":
                image_url = self.getdef_image(self.text, self.dictionaryname, \
                        "png", self.imagewidth, self.imageheight, "Black", \
                        self.fontsize)
                self.response.response_code = "303 see other"
                self.response.header = [('Location', image_url)]
            elif self.imageyn.lower() == "w":
                image_url = self.get_wiktionary_def_image(self.text, \
                        self.dictionaryname, "png", self.imageheight, \
                        self.imagewidth, "Black", self.fontsize)
                self.response.response_code = "303 See other"
                self.response.header = [('Location', image_url)]
            else:
    ...
    def get_json_result(self):
        ...
        try:
            if self.request.get('word'):
                definition = self.getdef(self.request.get('word'), \
    ...
    def get_wiktionary_def_image(self, word, dictionary, file_type='png', \
        ...
        if meaning == None:
            ...
            return renderer.render_text(meaning, file_type, 0, 0, color, \
                font_size=fontsize)
